...
function fromArray32(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    res = res.mul(radix).add(toBN(arr[i]));
  }
  return res;
}
...
function fnvHash(str) {
  const uint64_max = toBN(2).pow(toBN(64));
  let hash = toBN('CBF29CE484222325', 16);
  for (var i = 0; i < str.length; i++) {
    hash = hash.xor(toBN(str[i].charCodeAt()));
    hash = hash.mul(toBN(0x100000001b3));
