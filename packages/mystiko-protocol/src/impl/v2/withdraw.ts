...
import { Proof, CompilationArtifacts, ZoKratesProvider } from 'zokrates-js';
import {
  check,
  logger,
  readCompressedFile,
...
import { buffToBigInt, decryptAsymmetric, MERKLE_TREE_LEVELS, RANDOM_SK_SIZE, serialNumber } from '../v1';
import { v1Protocol } from '../index';
...
export async function zkProveWithdraw(
  ...
  const computedCommitmentHash = await v1Protocol.commitment(
    pkVerify,
    pkEnc,
    amount,
  ...
  const inputs = [
    ...
    sn.toString(),
    amount.toString(),
    toBN(toHexNoPrefix(recipient), 16).toString(),
    // private inputs
    buffToBigInt(pkVerify).toString(),
    buffToBigInt(skVerify).toString(),
    buffToBigInt(randomP).toString(),
    buffToBigInt(randomR).toString(),
    buffToBigInt(randomS).toString(),
    pathElements.map((e) => e.toString()),
    pathIndices.map((p) => p === 1),
