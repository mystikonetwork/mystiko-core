...
test('test bigIntToBuff', () => {
  ...
  expect(() => DefaultProtocol.bigIntToBuff(toBN(4022250938), 1)).toThrow();
  expect(DefaultProtocol.bigIntToBuff(toBN(4022250938), 4).toString('hex')).toBe('baadbeef');
  expect(DefaultProtocol.bigIntToBuff(toBN(4022250938), 6).toString('hex')).toBe('baadbeef0000');
});
...
test('test poseidonHash', () => {
  expect(() => DefaultProtocol.poseidonHash(['data to be hashed', toBN(1)])).toThrow();
  expect(() => DefaultProtocol.poseidonHash([toBN(1), 'data to be hashed'])).toThrow();
  const h1 = DefaultProtocol.poseidonHash([toBN(1), toBN(2)]);
  const h2 = DefaultProtocol.poseidonHash([toBN(3), toBN(4)]);
  const h3 = DefaultProtocol.poseidonHash([toBN(1), toBN(2)]);
...
test('test commitment', async () => {
  await expect(DefaultProtocol.commitment('deadbeef', toBuff('baadbabe'), toBN(1))).rejects.toThrow();
  await expect(DefaultProtocol.commitment(toBuff('baadbabe'), 'deadbeef', toBN(1))).rejects.toThrow();
...
test('test commitmentWithShieldedAddress', async () => {
  await expect(DefaultProtocol.commitmentWithShieldedAddress(123, toBN(1))).rejects.toThrow();
  const rawSkVerify = DefaultProtocol.randomBytes(DefaultProtocol.VERIFY_SK_SIZE);
  const rawSkEnc = DefaultProtocol.randomBytes(DefaultProtocol.ENCRYPT_SK_SIZE);
  const pkVerify = DefaultProtocol.publicKeyForVerification(rawSkVerify);
  const pkEnc = DefaultProtocol.publicKeyForEncryption(rawSkEnc);
