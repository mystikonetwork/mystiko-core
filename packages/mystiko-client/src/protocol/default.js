...
export function poseidonHash(inputs) {
  ...
  inputs.forEach((input) => check(isBN(input), 'input should be instance of BN'));
...
export function bigIntToBuff(bigInt, numBytes = 32) {
  check(isBN(bigInt), 'bigInt should be instance of BN');
  let res = bigInt;
  let index = 0;
...
export async function commitment(
  ...
  check(!randomS || isBN(randomS), 'randomS should be instance of BN');
