...
test('Test bnToFixedBytes', () => {
  const bn1 = utils.toBN(0xdeadbeef);
...
test('Test fromDecimals', () => {
  expect(utils.fromDecimals(utils.toBN('1'), 4)).toBe(0.0001);
  expect(utils.fromDecimals(utils.toBN('1000000000000000000'))).toBe(1);
});
...
test('Test toFixedLenHex', () => {
  ...
  expect(utils.toFixedLenHex('0xdead', 4)).toBe('0x0000dead');
  expect(utils.toFixedLenHex(utils.toBN('dead', 16), 4)).toBe('0x0000dead');
...
test('Test toHex', () => {
  ...
  expect(utils.toHex('0Xdead')).toBe('0xdead');
  expect(utils.toHex(utils.toBN('dead', 16))).toBe('0xdead');
...
test('Test toHexNoPrefix', () => {
  ...
  expect(utils.toHexNoPrefix('0Xdead')).toBe('dead');
  expect(utils.toHexNoPrefix(utils.toBN('dead', 16))).toBe('dead');
...
test('Test toFixedLenHexNoPrefix', () => {
  ...
  expect(utils.toFixedLenHexNoPrefix('0xdead', 4)).toBe('0000dead');
  expect(utils.toFixedLenHexNoPrefix(utils.toBN('dead', 16), 4)).toBe('0000dead');
...
test('Test toBN', () => {
  expect(utils.toString(utils.toBN('1234'))).toBe('1234');
});

test('Test isBN', () => {
  expect(utils.isBN(undefined)).toBe(false);
  expect(utils.isBN('123')).toBe(false);
  expect(utils.isBN(utils.toBN('1230000'))).toBe(true);
});
