...
export class Deposit extends BaseModel {
  ...
  set amount(amnt) {
    check(isBN(amnt), 'amnt should be instance of BN');
  ...
  set commitmentHash(hash) {
    check(isBN(hash), 'hash should be instance of Buffer');
  ...
  set randomS(s) {
    check(isBN(s), 's should be instance of BN');
  ...
  set hashK(k) {
    check(isBN(k), 'k should be instance of BN');
