...
export class MerkleTree {
  constructor(maxLevels, initialElements = [], { zeroElement = undefined } = {}) {
    ...
    check(!zeroElement || isBN(zeroElement), 'zeroElement should be an instance of BN');
    initialElements.forEach((element) => {
      check(isBN(element), 'element should be an instance of BN');
    });
  ...
  static hash2(first, second) {
    check(isBN(first), 'first must be an instance of BN');
    check(isBN(second), 'second must be an instance of BN');
  ...
  static calcZeros(firstZero, levels) {
    check(isBN(firstZero), 'firstZero must be an instance of BN');
