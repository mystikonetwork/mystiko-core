...
class TestModel extends BaseModel {
  ...
  get bn() {
    return this.data['bn'] ? new BN(this.data['bn']) : undefined;
  }
}
...
test('Test columnComparator', () => {
  let model1 = new TestModel({ [ID_KEY]: 1, col: -1, bn: new BN(3) });
  let model2 = new TestModel({ [ID_KEY]: 2, col: 2, bn: new BN(1) });
  let model3 = new TestModel({ [ID_KEY]: 3, col: undefined, bn: undefined });
  let model4 = new TestModel({ [ID_KEY]: 4, col: null, bn: null });
  let model5 = new TestModel({ [ID_KEY]: 5, col: 3, bn: new BN(2) });
  let model6 = new TestModel({ [ID_KEY]: 6, col: 3, bn: new BN(2) });
  let model7 = new TestModel({ [ID_KEY]: 7, col: 1, bn: new BN(100) });
