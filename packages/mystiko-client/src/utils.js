...
export function fromDecimals(bn, decimals = 18) {
  check(isBN(bn), 'bn should be an instance of BN');
  check(typeof decimals === 'number', 'decimals should be a number');
 ...
 * @param {number|string|number[]|Uint8Array|Buffer|external:BN} number the object to be converted
 * @param {number|string} [base] convert base
 * @param {string} [endian] endianness
 * @returns {external:BN} the converted BN instance.
 */
export function toBN(number, base = undefined, endian = undefined) {
